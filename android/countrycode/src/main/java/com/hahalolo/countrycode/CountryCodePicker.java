package com.hahalolo.countrycode;
...
public class CountryCodePicker extends RelativeLayout {
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null && xmlWidth != null
                && (xmlWidth.equals(LayoutParams.MATCH_PARENT + "")
                || xmlWidth.equals(LayoutParams.FILL_PARENT + "")
                || xmlWidth.equals("fill_parent")
                || xmlWidth.equals("match_parent"))) {
            holderView = mInflater.inflate(R.layout.country_code_layout_full_width_code_picker, this, true);
        } else {
            holderView = mInflater.inflate(R.layout.country_code_layout_code_picker, this, true);
        }
    ...
    private void applyCustomProperty(AttributeSet attrs) {
        ...
        try {
            ...
            if (isInEditMode()) {
                ...
                contentColor = a.getColor(R.styleable.CountryCodePicker_ccp_contentColor, context.getResources().getColor(R.color.text_body));
            }
            ...
            if (isInEditMode()) {
                ...
                borderFlagColor = a.getColor(R.styleable.CountryCodePicker_ccp_flagBorderColor, context.getResources().getColor(R.color.transparent));
            }
    ...
    void setSelectedCountry(CCPCountry selectedCCPCountry) {
        ...
        Log.d("CountryCodePicker", "Display Text: " + displayText);
    ...
    private void updateHint() {
        if (editText_registeredCarrierNumber != null && hintExampleNumberEnabled) {
            ...
            Phonenumber.PhoneNumber exampleNumber = getPhoneUtil().getExampleNumberForType(
                    getSelectedCountryNameCode(), getSelectedHintNumberType());
            if (exampleNumber != null) {
                formattedNumber = exampleNumber.getNationalNumber() + "";
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    formattedNumber = PhoneNumberUtils.formatNumber(
                            getSelectedCountryCodeWithPlus() + formattedNumber, getSelectedCountryNameCode());
                } else {
                    formattedNumber = PhoneNumberUtils.formatNumber(
                            getSelectedCountryCodeWithPlus() + formattedNumber);
                }
    ...
    public void resetToDefaultCountry() {
        defaultCCPCountry = CCPCountry.getCountryForNameCodeFromLibraryMasterList(
                getContext(), getLanguageToApply(), getDefaultCountryNameCode());
        setSelectedCountry(defaultCCPCountry);
    }
    ...
    public void setCountryForPhoneCode(int countryCode) {
        ...
        CCPCountry ccpCountry = CCPCountry.getCountryForCode(getContext(),
                getLanguageToApply(), preferredCountries, countryCode);
        if (ccpCountry == null) {
            if (defaultCCPCountry == null) {
                defaultCCPCountry = CCPCountry.getCountryForCode(getContext(),
                        getLanguageToApply(), preferredCountries, defaultCountryCode);
            }
            setSelectedCountry(defaultCCPCountry);
        } else {
    ...
    public void setCountryForNameCode(String countryNameCode) {
        //xml stores data in string format, but want to allow only numeric value to country code to user.
        CCPCountry country = CCPCountry.getCountryForNameCodeFromLibraryMasterList(getContext(),
                getLanguageToApply(), countryNameCode);
        if (country == null) {
            if (defaultCCPCountry == null) {
                defaultCCPCountry = CCPCountry.getCountryForCode(getContext(),
                        getLanguageToApply(), preferredCountries, defaultCountryCode);
    ...
    public boolean isValidFullNumber() {
        try {
            if (getEditText_registeredCarrierNumber() != null && getEditText_registeredCarrierNumber().getText().length() != 0) {
                Phonenumber.PhoneNumber phoneNumber = getPhoneUtil().parse("+" + selectedCCPCountry.getPhoneCode() +
                        getEditText_registeredCarrierNumber().getText().toString(), selectedCCPCountry.getNameCode());
