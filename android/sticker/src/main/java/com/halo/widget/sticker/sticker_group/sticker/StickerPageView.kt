...
class StickerPageView : FrameLayout, LifecycleOwner {
    ...
    private fun initHandleSticker() {
        listener?.run {
            this.stickerRepository().stickerPacks(this.token()).observe(this.lifecycleOwner()){
                stickerPagerAdapter?.updateList(it?: mutableListOf())
                initTabLayout()
            }
        }
    }
