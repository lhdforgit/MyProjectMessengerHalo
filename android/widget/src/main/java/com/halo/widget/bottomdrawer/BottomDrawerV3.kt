...
class BottomDrawerV3 : FrameLayout {
    ...
    override fun onFinishInflate() {
        ...
        setBackgroundColor(ActivityCompat.getColor(context, R.color.primary))
    }

    private fun initAttributes(context: Context, attrs: AttributeSet?) {
        var attr: TypedArray? = null
        try {
            attr = context.obtainStyledAttributes(attrs, R.styleable.BottomDrawerV3, 0, 0)
            extraPadding = attr.getDimensionPixelSize(
                R.styleable.BottomDrawerV3_bottom_sheet_extra_padding_v3,
                resources.getDimensionPixelSize(R.dimen.bottom_sheet_extra_padding)
            )

            cornerRadius = attr.getDimensionPixelSize(
                R.styleable.BottomDrawerV3_bottom_sheet_corner_radius_v3,
            ...
            shouldDrawUnderStatus = attr.getBoolean(
                R.styleable.BottomDrawerV3_should_draw_under_status_bar_v3,
                false
            )

            shouldDrawUnderHandle = attr.getBoolean(
                R.styleable.BottomDrawerV3_should_draw_content_under_handle_view_v3,
                false
            )

            drawerBackground = attr.getColor(
                R.styleable.BottomDrawerV3_bottom_drawer_background_v3,
