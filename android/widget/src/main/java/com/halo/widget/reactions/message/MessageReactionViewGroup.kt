...
class MessageReactionViewGroup(context: Context, private val config: ReactionsConfig) :
    ViewGroup(context) {

    private val tag = MessageReactionViewGroup::class.java.simpleName
    ...
    private var bulletSize: Int = SizeUtils.dp2px(4f)
    private var bulletPadding: Int = SizeUtils.dp2px(2f)
    ...
    private var quoteWidth = mediumIconSize + verticalPadding
    ...
    private val bulletReactions: List<ImageView> = reactions.map {
        ImageView(context).apply {
            this.layoutParams = LayoutParams(bulletSize, bulletSize)
            this.setImageResource(R.drawable.ic_bullet_reaction)
            addView(this)
        }
    }

    private val quoteView = QuoteView(context).also {
//        if (messageMenuListener?.enableQuote() == true) {
        it.layoutParams = LayoutParams(mediumIconSize, mediumIconSize)
        addView(it)
...
//        }
    ...
    private val clearReacView = QuoteView(context).also {
//        if (messageMenuListener?.enableQuote() == true) {
        it.layoutParams = LayoutParams(mediumIconSize, mediumIconSize)
        it.setImageResource(R.drawable.ic_clear_reac)
        addView(it)
        it.setOnClickListener {
            messageMenuListener?.onClearReaction()
            dismiss()
            dismissListener
        }
//        }
    ...
    override fun onSizeChanged(width: Int, height: Int, oldW: Int, oldH: Int) {
        ...
        if (dialogX + dialogWidth + quoteWidth >= width) {
            // Center dialog
            // dialogX = Math.max(0, (width - dialogWidth) / 2)
            // Right dialog
            dialogX = Math.max(
                0,
                (width - (dialogWidth + horizontalPadding + mediumIconSize / 2 + quoteWidth))
            )
        } else if (dialogX < horizontalPadding) dialogX = horizontalPadding
        ...
        if (dialogY < 0) {
            ...
            dialogY = parentLocation.y + parentSize.second
        }
    }
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        background.also { view ->
            ...
            val bottomView = Math.min(dialogY, firstClick.y) + dialogHeight + translationY
            val bottom = Math.min(height, bottomView)
            val top = bottom + mediumIconSize - view.layoutParams.height - dialogHeight
            val bonusBottom = (if (messageMenuListener?.haveReaction()==true ) (bulletSize + bulletPadding) else 0 )

            view.layout(left, top, right, bottom + bonusBottom)
            bgWidth = view.width
        }

        quoteView.takeIf { messageMenuListener?.enableQuote() == true }?.apply {
            this.visibility = View.VISIBLE
            val left = this@MessageReactionViewGroup.background.right + verticalPadding
            val top = this@MessageReactionViewGroup.background.top
            val right = left + mediumIconSize
            val bottom = this@MessageReactionViewGroup.background.bottom
        ...
        clearReacView.takeIf { messageMenuListener?.enableClearReaction() == true }?.apply {
            this.visibility = View.VISIBLE
            val left = this@MessageReactionViewGroup.background.right + verticalPadding
            val top = this@MessageReactionViewGroup.background.top
            val right = left + mediumIconSize
        ...
        reactions.forEachIndexed { index, view ->
            ...
            val translationY = view.translationY.toInt()
            val bottomView =
                Math.min(dialogY, firstClick.y) + dialogHeight - verticalPadding + translationY
            val bottom = Math.min(height - verticalPadding, bottomView)
            ...
            bulletReactions[index].apply {
                this.visibility = if (messageMenuListener?.enableReaction(index) == true) View.VISIBLE else View.INVISIBLE
                this.layout(
                    (left + right) / 2 - bulletSize / 2,
                    bottom + bulletPadding,
                    (left + right) / 2 + bulletSize / 2,
                    bottom + bulletPadding + bulletSize
                )
            }
        }
        ...
        if (reactionText.visibility == View.VISIBLE) {
            ...
            val top = selectedView.top - Math.min(
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN, MotionEvent.ACTION_MOVE -> {
                // Ngăn xự kiện move trên view cha
                config.reactionButton?.parent?.requestDisallowInterceptTouchEvent(true)
            ...
            MotionEvent.ACTION_UP -> {
                config.reactionButton?.parent?.requestDisallowInterceptTouchEvent(false)
    ...
    private fun getIntersectedIcon(x: Float, y: Float): ReactionView? =
        reactions.firstOrNull {
            x >= it.location.x - horizontalPadding
                    ...
                    && y >= it.location.y - (horizontalPadding + it.height)
