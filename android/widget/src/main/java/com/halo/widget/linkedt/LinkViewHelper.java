...
/**
 * Utility class to help implement {@link LinkView} on any {@link TextView}-based class.
 * This class itself is not a {@link View}.
 */
public final class LinkViewHelper implements LinkView {
    private static final int FLAG_HASHTAG = 1;
    private static final int FLAG_MENTION = 2;
    private static final int FLAG_HYPERLINK = 4;

    public static final String LINK_REGEX = "^(http:\\/\\/www\\.|https:\\/\\/www\\.|http:\\/\\/|https:\\/\\/)?[a-z0-9]+([\\-\\.]{1}[a-z0-9]+)*\\.[a-z]{2,5}(:[0-9]{1,5})?(\\/.*)?$";
                ...
                switch (c) {
                    ...
                    default:
                        if (!Character.isLetterOrDigit(c)) {
                            ...
                            hashtagChangedListener.onChanged(LinkViewHelper.this, s.subSequence(
                                    indexOfPreviousNonLetterDigit(s, 0, start - 1) + 1, start
                            ));
                        } else if (mentionChangedListener != null && mentionEditing) {
                            mentionChangedListener.onChanged(LinkViewHelper.this, s.subSequence(
                                    indexOfPreviousNonLetterDigit(s, 0, start - 1) + 1, start
                            ));
                        }
                        break;
    ...
    /**
     * Configuring {@link LinkView} into given view.
     *
     * @param view TextView to install SocialView into.
     */
    public LinkViewHelper(@NonNull TextView view) {
        this(view, null);
    }
     ...
     */
    public LinkViewHelper(@NonNull TextView view, @Nullable AttributeSet attrs) {
        this.view = view;
        ...
        final TypedArray a = view.getContext().obtainStyledAttributes(
                attrs, R.styleable.LinkView, R.attr.socialViewStyle, R.style.Widget_LinkView
        );
        flags = a.getInteger(R.styleable.LinkView_socialFlags, FLAG_HASHTAG | FLAG_MENTION | FLAG_HYPERLINK);
        hashtagColors = a.getColorStateList(R.styleable.LinkView_hashtagColor);
        mentionColors = a.getColorStateList(R.styleable.LinkView_mentionColor);
        hyperlinkColors = a.getColorStateList(R.styleable.LinkView_hyperlinkColor);
        a.recycle();
        recolorize();
    }
    ...
    public Pattern getHyperlinkPattern() {
        return hyperlinkPattern != null ? hyperlinkPattern : PatternsCompat.AUTOLINK_WEB_URL;
    }
     ...
     * when {@link LinkView.OnClickListener} are activated.
     */
    private static class SocialClickableSpan extends ClickableSpan {
        ...
        public void onClick(@NonNull View widget) {
            if (!(widget instanceof LinkView)) {
                throw new IllegalStateException("Clicked widget is not an instance of SocialView.");
            }
            listener.onClick((LinkView) widget, !isHyperlink ? text.subSequence(1, text.length()) : text);
        }
