 ...
 */
public abstract class LinkArrayAdapter<T> extends ArrayAdapter<T> {
    private Filter filter;
    private final List<T> tempItems = new ArrayList<>();

    public LinkArrayAdapter(@NonNull Context context, int resource, int textViewResourceId) {
    ...
    private class SocialFilter extends Filter {
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            if (results.values instanceof List) {
                ...
                if (results.count > 0) {
                    LinkArrayAdapter.super.clear();
                    for (final T object : list) {
                        LinkArrayAdapter.super.add(object);
                    }
