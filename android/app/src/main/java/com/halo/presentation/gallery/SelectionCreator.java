...
import com.halo.presentation.gallery.entities.CaptureStrategy;
...
public final class SelectionCreator {
     ...
     *
     * @param media     a requester context wrapper.
     * @param mimeTypes MIME type set to select.
     */
    SelectionCreator(Media media, @NonNull Set<MimeType> mimeTypes, boolean mediaTypeExclusive) {
        mMedia = media;
     ...
     *
     * @param themeId theme resource id. Default {@see R.style.Halo_Theme}
     * @return {@link SelectionCreator} for fluent API.
     */
    ...
    /**
     * Determines whether the photo capturing is enabled, and handle capture now when start
     * <p>
     * If this value is set true, photo capturing will handle capture now when start
     *
     * @param now Whether to capture now. Default value is false;
     * @return {@link SelectionCreator} for fluent API.
     */
    public SelectionCreator captureNow(boolean now) {
        mSelectionSpec.captureNow = now;
        return this;
    }
    ...
    /**
     * Set dafault item of media when reopen media
     *
     * @param items List {@link Item} media
     * @return @return {@link SelectionCreator} for fluent API.
     */
    public SelectionCreator setDefaultItemSelection(List<Item> items) {
        mSelectionSpec.defaultItemSelection = items;
        return this;
    }
    ...
    public void forResult(int requestCode) {
        Activity activity = mMedia.getActivity();
        if (activity == null) {
            return;
        }

        Intent intent = MediaAct.getIntent(activity);

        Fragment fragment = mMedia.getFragment();
        if (fragment != null) {
