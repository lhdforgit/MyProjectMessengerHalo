...
import com.halo.editor.mediasend.MediaSendConstants;
...
import com.halo.stream.Collectors;
...
class ShareRepository {
    ...
    private static final String TAG = ShareRepository.class.getName();

    /**
     * Handles a single URI that may be local or external.
     */
    void getResolved(Context context, @Nullable Uri uri, @Nullable String mimeType, @NonNull Callback<Optional<ShareData>> callback) {
        EditorExecutors.BOUNDED.execute(() -> {
            try {
                callback.onResult(Optional.of(getResolvedInternal(uri, mimeType, context)));
            } catch (IOException e) {
     ...
     */
    void getResolved(Context context, @NonNull List<Uri> uris, @NonNull Callback<Optional<ShareData>> callback) {
        EditorExecutors.BOUNDED.execute(() -> {
            try {
                callback.onResult(Optional.fromNullable(getResolvedInternal(uris, context)));
            } catch (IOException e) {
                Log.w(TAG, "Failed to resolve!", e);
                callback.onResult(Optional.absent());
            }
    ...
    @WorkerThread
    private @NonNull
    ShareData getResolvedInternal(@Nullable Uri uri, @Nullable String mimeType, Context context) throws IOException {
    ...
    @WorkerThread
    private @Nullable
    ShareData getResolvedInternal(@NonNull List<Uri> uris, Context context) throws IOException {
    ...
    String getMimeType(@NonNull Context context, @NonNull Uri uri, @Nullable String mimeType) {
    ...
    String getFileName(@NonNull Context context, @NonNull Uri uri) {
        try (Cursor cursor = context.getContentResolver().query(uri, null, null, null, null)) {
