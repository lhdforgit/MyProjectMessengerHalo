...
public class BlobProvider {
    ...
    public static final Uri CONTENT_URI = Uri.parse("content://com.hahalolo.android.social/blob");
    public static final String AUTHORITY = "com.hahalolo.android.social";
    ...
    public synchronized void delete(@NonNull Context context, @NonNull Uri uri) {
        if (!isAuthority(uri)) {
            Timber.d("Can't delete. Not the authority for uri: %s", uri);
            return;
        }
        ...
        try {
            ...
            Timber.w(e, "Failed to delete uri: %s", uri);
        }
    }
    ...
    @WorkerThread
    private synchronized @NonNull
    Uri writeBlobSpecToDisk(@NonNull Context context,
                            @NonNull BlobSpec blobSpec) throws IOException {
    ...
    Uri writeBlobSpecToDiskAsync(@NonNull Context context,
                                 ...
                                 @Nullable ErrorListener errorListener) throws IOException {
        ...
        EditorExecutors.UNBOUNDED.execute(() -> {
            try {
    ...
    public class BlobBuilder {
         ...
         */
        @WorkerThread
        public Uri createForSingleSessionOnDiskAsync(
                @NonNull Context context,
                @Nullable SuccessListener successListener,
                @Nullable ErrorListener errorListener) throws IOException {
            return writeBlobSpecToDiskAsync(
                    context,
                    buildBlobSpec(StorageType.SINGLE_SESSION_DISK),
                    successListener,
                    errorListener);
        }
         ...
         */
        @WorkerThread
        public Uri createForMultipleSessionsOnDiskAsync(
                @NonNull Context context,
                @Nullable SuccessListener successListener,
                @Nullable ErrorListener errorListener) throws IOException {
            return writeBlobSpecToDiskAsync(
                    context,
                    buildBlobSpec(StorageType.MULTI_SESSION_DISK),
                    successListener,
                    errorListener);
        }
    }
