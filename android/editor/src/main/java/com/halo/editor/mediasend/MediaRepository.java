...
import com.halo.editor.glide.PartAuthority;
import com.halo.editor.permissions.Permissions;
import com.halo.editor.util.EditorExecutors;
...
public class MediaRepository {
    ...
    void getFolders(@NonNull Context context, @NonNull Callback<List<MediaFolder>> callback) {
        EditorExecutors.BOUNDED.execute(() -> callback.onComplete(getFolders(context)));
    }
    ...
    public void getMediaInBucket(@NonNull Context context, @NonNull String bucketId, @NonNull Callback<List<Media>> callback) {
        EditorExecutors.BOUNDED.execute(() -> callback.onComplete(getMediaInBucket(context, bucketId)));
    }
    ...
    void getPopulatedMedia(@NonNull Context context, @NonNull List<Media> media, @NonNull Callback<List<Media>> callback) {
        ...
        EditorExecutors.BOUNDED.execute(() -> callback.onComplete(getPopulatedMedia(context, media)));
    }

    void getMostRecentItem(@NonNull Context context, @NonNull Callback<Optional<Media>> callback) {
        EditorExecutors.BOUNDED.execute(() -> callback.onComplete(getMostRecentItem(context)));
    }
    ...
    static void transformMedia(@NonNull Context context,
        ...
        EditorExecutors.BOUNDED.execute(() -> callback.onComplete(transformMedia(context, currentMedia, modelsToTransform)));
    }

    @WorkerThread
    private @NonNull
    List<MediaFolder> getFolders(@NonNull Context context) {
    ...
    @WorkerThread
    private @NonNull
    FolderResult getFolders(@NonNull Context context, @NonNull Uri contentUri) {
    ...
    @WorkerThread
    private @NonNull
    List<Media> getMediaInBucket(@NonNull Context context, @NonNull String bucketId) {
        if (!Permissions.hasAll(context, Manifest.permission.READ_EXTERNAL_STORAGE)) {
            return Collections.emptyList();
        }
    ...
    @WorkerThread
    private @NonNull
    List<Media> getMediaInBucket(@NonNull Context context, @NonNull String bucketId, @NonNull Uri contentUri, boolean isImage) {
        ...
        try (Cursor cursor = context.getContentResolver().query(contentUri, projection, selection, selectionArgs, sortBy)) {
            while (cursor != null && cursor.moveToNext()) {
                ...
                media.add(new Media(null, uri, mimetype, date, width, height, size, duration, Optional.of(bucketId), null, false));
            }
        }
    ...
    private Media getLocallyPopulatedMedia(@NonNull Context context, @NonNull Media media) throws IOException {
        ...
        if (size <= 0) {
            if (media.getUri() != null) {
                Optional<Long> optionalSize = Optional.fromNullable(PartAuthority.getAttachmentSize(media.getUri()));
        ...
        return new Media(null, media.getUri(), media.getMimeType(), media.getDate(), width, height, size, 0, media.getBucketId(), media.getCaption(), media.getBorderless());
    }
