...
class Camera1Controller {
  ...
  void initialize() {
    ...
    if (Camera.getNumberOfCameras() <= 0) {
      Timber.tag(TAG).w("Device doesn't have any cameras.");
      onCameraUnavailable();
      return;
    }
    ...
    try {
      ...
      Timber.tag(TAG).w(e, "Failed to open camera.");
      onCameraUnavailable();
      return;
    }

    if (camera == null) {
      Timber.tag(TAG).w("Null camera instance.");
      onCameraUnavailable();
      return;
    }
    ...
    Timber.d("Preview size: " + previewSize.width + "x" + previewSize.height + "  Picture size: " + pictureSize.width + "x" + pictureSize.height);
  ...
  void linkSurface(@NonNull SurfaceTexture surfaceTexture) {
    Timber.d("linkSurface() called");
    enforcer.run(Stage.INITIALIZED, () -> {
      try {
        Timber.d("linkSurface() executing");
        ...
        Timber.tag(TAG).w(e, "Failed to start preview.");
  ...
  void setScreenRotation(int screenRotation) {
    Timber.d("setScreenRotation(" + screenRotation + ") called");
    enforcer.run(Stage.PREVIEW_STARTED, () -> {
      ...
      Timber.d("Preview rotation: " + previewRotation + "  Output rotation: " + outputRotation);
