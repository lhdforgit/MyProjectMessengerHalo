...
import com.halo.editor.imageeditor.ColorableRenderer;
import com.halo.editor.imageeditor.ImageEditorView;
...
import com.halo.editor.imageeditor.renderers.FaceBlurRenderer;
import com.halo.editor.imageeditor.renderers.MultiLineTextRenderer;
import com.halo.editor.mediasend.send.MediaSendPageFragment;
...
import com.halo.editor.scribbles.widget.VerticalSlideColorPicker;
...
import com.halo.editor.util.PushMediaConstraints;
import com.halo.editor.util.SaveAttachmentTask;
...
public final class ImageEditorFragment extends Fragment
        implements ImageEditorHud.EventListener,
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (editorModel == null) {
            editorModel = isAvatarMode ? EditorModel.createForCircleEditing()
                    : (isCoverMode ? EditorModel.createForCoverEditing() : EditorModel.create());
            EditorElement image = new EditorElement(new UriGlideRenderer(
                    imageUri, true, imageMaxWidth, imageMaxHeight));
    ...
    public void restoreState(@NonNull Object state) {
        if (state instanceof Data) {
            ...
            Timber.w( "Received a bad saved state. Received class: %s", state.getClass().getName());
        }
    }
    ...
    private void startTextEntityEditing(@NonNull EditorElement textElement, boolean selectAll) {
        imageEditorView.startTextEditing(textElement, false, selectAll);
    }
        ...
        SimpleTask.run(getLifecycle(), () -> {
            if (mainImage.getRenderer() != null) {
                ...
                if (bitmap != null) {
                    ...
                    Bitmap render = model.render(requireContext().getApplicationContext(), size);
                    try {
            ...
            Permissions.with(this)
                    ...
                    .onAllGranted(() -> {
                        SimpleTask.run(() -> {
                            ...
                            saveTask.executeOnExecutor(EditorExecutors.BOUNDED, attachment);
                        });
                    })
                    .execute();
        });
