...
import com.hahalolo.player.*
import com.hahalolo.player.core.Binder.Options
import com.hahalolo.player.core.MemoryMode.*
import com.hahalolo.player.core.Playback.Config
...
import com.hahalolo.player.internal.MasterNetworkCallback
  ...
  internal fun play(playable: Playable) {
    ...
    if (plannedManualPlayables.contains(tag)) {
      requireNotNull(playable.playback).also {
        if (!requireNotNull(it.config.controller).playerCanPause()) {
