...
import com.hahalolo.messenger.databinding.ChatLinkOutcommingItemBinding

class ChatLinkOutComingItem(binding: ChatLinkOutcommingItemBinding,
                            chatListener: ChatListener,
                            holderListener: HolderListener,
                            imageLoader: ImageLoader,
                            style: MessagesListStyle
)
    : ChatOutComingItem<ChatLinkOutcommingItemBinding>(binding, chatListener, holderListener, imageLoader, style) {

    companion object {
        fun create(parent: ViewGroup,
                   chatListener: ChatListener,
                   holderListener: HolderListener,
                   imageLoader: ImageLoader, style: MessagesListStyle): ChatLinkOutComingItem {
            val binding = DataBindingUtil.inflate<ChatLinkOutcommingItemBinding>(LayoutInflater.from(parent.context),
                    R.layout.chat_link_outcomming_item,
                    parent,
                    false)
            return ChatLinkOutComingItem(binding!!, chatListener, holderListener, imageLoader, style)
        }
    }

    override fun initView(binding: ChatLinkOutcommingItemBinding) {
